...
const MiMCOPRUChallenge = artifacts.require('MiMCOPRUChallenge');
...
contract('MiMC roll up test', async accounts => {
  ...
  it.only('Challenge test should return the correct merkle root after appending some items', async () => {
    let merkleProof = await merkleTree.path(0);
    let items = [1];
    let prevRoot = await merkleTree.root();
    for (let i = 0; i < items.length; i++) {
    ...
    let newRoot = await merkleTree.root();
    let mimc = await MiMC.deployed();
    await MiMCOPRUChallenge.link('MiMC', mimc.address);
    let mimcChallenger = await MiMCOPRUChallenge.new(prevRoot, 0);
    await mimcChallenger.addItems(items, merkleProof.path_elements, { gas: 6700000 });
    let challengeRoot = await mimcChallenger.root();
    newRoot.should.equal(challengeRoot.toString());
  });
});
